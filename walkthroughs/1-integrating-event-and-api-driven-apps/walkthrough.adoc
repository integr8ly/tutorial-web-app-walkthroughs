// tag::master-1[]
:walkthrough: Integrating message-oriented middleware with an API using AMQ
...
This walkthrough demonstrates a pattern that integrates a message driven architecture with an API driven architecture. 

Message-oriented middleware (MOM)::
MOM allows you implement an asynchronous, routable and transformable message queue.

API-centric architecture::
Typically, a developer working on a back-end application implements a synchronous API to allow communication from other applications.
...
This is not trivial because of the conflicting underlying timing models. 

In this walkthrough, you create a simple and intuitive integration with built-in monitoring that enables communication between two applications:

* A link:{route-frontend-host}[Node.js app, window="_blank"] provides a method to create messages, in this example, a new inventory item.
* A link:{route-crud-host}[Spring Boot CRUD app, window="_blank"] provides an API, in this example, an inventory API.

To create this integration, you use link:{amq-url}[AMQ Online, window="_blank"] and API connectors in link:{fuse-url}[Red Hat Fuse Online, window="_blank"].
...
Fuse Online provides connectors for many services, for example, Gmail and IRC.
However, each API must be registered as a separate customization.
...
To send messages to the Spring Boot CRUD app, you create a connection in Red Hat Fuse Online using the API connector you just created.
...
To receive messages, you create a connection in Red Hat Fuse Online using the AMQ connector.
