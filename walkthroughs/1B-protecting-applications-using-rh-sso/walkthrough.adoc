...
. Details for teh `{user-username}` realm are displayed upon login success.
...
[type=verification]
Select the *Clients* item in the side menu. 
Is the `order-entry-system` client listed and *Enabled* set to `True`?
