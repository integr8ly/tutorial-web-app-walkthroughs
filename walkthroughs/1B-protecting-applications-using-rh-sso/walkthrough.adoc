...
.. Enter `{user-username}` in the *Client ID* field.
...
.. Click *Save*.
. The *Settings* screen for the `{user-username}` client should be displayed.
...
Is the `{user-username}` client listed and is the *Enabled* field set to `True`?
...
. Click the `{user-username}` client that was created earlier.
