...
NOTE: Enabling the adapter can vary depending on how an application has been coded. The Order Entry System has been developed to enable SSO protection using the `keycloak-connect` Node.js module if a configuration is found in the environment on startup.
