...
.. Paste the URL of the *Order Entry System UI* from the *Integrating message-oriented middleware with a RESTful API using AMQ Online* Solution Pattern in the *Root URL* field. This should look similar to `https://order-entry-ui-{user-sanitized-username}-<NAMESPACE>.{openshift-app-host}`
.. Click *Save*.
...
+
NOTE: Enabling the adapter can vary depending on how an application has been coded. The Order Entry System has been developed to enable SSO protection if a configuration is found in the environment on startup.
...
. Select the *walkthroughs-1A-integrate-event-and-api-driven-apps* project.
...
. Open the URL listed beside the *rhmi-lab-nodejs-order-frontend* in either a private browser session, or a different browser to view the *Order Entry System* UI. A login screen with the title *{user-username} Realm* is displayed.
+
NOTE: We suggest using a private session or different browser to ensure any old sessions that might exist are not being used.
