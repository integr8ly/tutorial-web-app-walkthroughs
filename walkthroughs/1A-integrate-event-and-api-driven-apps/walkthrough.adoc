...
In this Solution Pattern, you create a simple and intuitive integration with built-in monitoring, to enable communication between two applications:
...
+
NOTE: When entering the password above the input is not shown in the terminal for security reasons. Type or paste the password then press the `Enter` key and it will be accepted.
